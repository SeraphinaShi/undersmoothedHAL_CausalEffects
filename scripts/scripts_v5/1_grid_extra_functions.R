run_simu_scaled_rep_extra <- function(gen_data_func, eval_points, y_type, n, rounds, return_all_rslts = F){
  ...
  for(r in 1:rounds){
    ...
    result <- tryCatch({
      run_simu_1round_scalers_extra(gen_data_func, eval_points, y_type, n=n, lambda_scalers=lambda_scalers)
    }, error = function(e) {
...
run_simu_1round_scalers_extra <- function(gen_data_func, eval_points, y_type, n, lambda_scalers){
  ...
  for (i in 1:length(lambda_scalers)) {
    ...
    if(length(nonzero_idx) > 0) {
      ...
      psi_hat_pnt_scaled <- cbind(eval_points, matrix(psi_hat, ncol=1), lambda, lambda_scaler, hal_scaled_fit_time, num_basis)
