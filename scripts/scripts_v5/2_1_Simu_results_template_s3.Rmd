...
p <- plot_perforences_alllambda_noBT(df = results_grid_sum$result_summary,
                                u_g_scaler=u_g_scaler, u_l_scalers=u_l_scalers, 
                                save_plot=here(getwd(), "plots", paste0("simu_sys", sn, "_n", n, "_performences_grid.png")), 
                                max_bias_sd=NA)

no_empirical_CI_proportion <- results_grid_sum$no_empirical_CI_proportion
...
print(unique(results_so$result_summary[, c('smooth_order', 'lambda', 'n_basis', 'sl_pick')]))
...
```{r fig.height=9, fig.width=9}
p <- plot_performences_cv_u_gl_alla_noBT(df = results$result_summary, save_plot = here(getwd(), "plots", paste0("simu_sys", sn, "_n", n, "_performences.png")))
```
