...
for (i in 1:length(a_vec)) {
  ...
  psi0[i] <- mean(data_a_0$Y - data_0_0$Y)
}
```
...
n <- 1000
Obs1 <- generate_data1(n)
summary(Obs1)
```

```{r}
psi_hat <- data.frame(targ_par = paste0("ATE(",a_vec,",0)"), 
                      psi0 = psi0,
                      ss_under_hal = NA,
                      ss_hal = NA)
...
for (i in 1:length(a_vec)) {
  a <- a_vec[i]
  ate_hal_relts <- cal_ate_hal(df=Obs1,
                               y_name = "Y", x_names = c("W", "A", "Z"),
                               y_type = "binomial",
                               a1 = a, a0 = 0, z = 0
                               )
  psi_hat$ss_under_hal = ate_hal_relts$psi_ss_under
  psi_hat$ss_hal = ate_hal_relts$psi_ss_init
